...
public class ListingAdapter extends MainAdapter {
    ...
    public int getItemViewType(int position) {
        if (ListingActivity.movieListingItems.get(position) instanceof Alphabet)
            return ALPHABET_ROW_ID;
        if (ListingActivity.movieListingItems.get(position) instanceof MovieCard)
            return MOVIE_CARD_ROW_ID;
        if (ListingActivity.movieListingItems.get(position) instanceof MovieCounter)
